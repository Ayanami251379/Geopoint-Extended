...
(function () {
  ...
  /**
   * Find center of coordinates array
   *
   * @param   {Object}    points        GeoPoint instance
   * @return  {Object}    point         GeoPoint instance
   */
  GeoPoint.findCenter = function(points) {
	...
	for(i=0; i<num_coords; i++){
        var lat = points[i].latitude() * Math.PI / 180;
        var lon = points[i].longitude() * Math.PI / 180;
  ...
  /**
   * Find maximum and minimum of coordinates array 
   *
   * @param   {Object}    points        GeoPoint instance
   * @return  {Array}     points        GeoPoint instance
   */
  GeoPoint.findMaxMin = function(points) {
	var num_coords = points.length;
	
	if (num_coords < 2) {
      throw new Error('Need more than one coordinate');
    }
    ...
    return [new GeoPoint(maxLat, maxLon), new GeoPoint(minLat, minLon)];
  };
